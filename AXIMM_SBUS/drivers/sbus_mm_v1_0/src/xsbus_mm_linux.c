...
typedef struct {
    ...
    XSbus_mm_uio_map maps[ MAX_UIO_MAPS ];
} XSbus_mm_uio_info;

/***************** Variable Definitions **************************************/
static XSbus_mm_uio_info uio_info;
...
static int uio_info_read_name(XSbus_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSbus_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSbus_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSbus_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSbus_mm_Initialize(XSbus_mm *InstancePtr, const char* InstanceName) {
	XSbus_mm_uio_info *InfoPtr = &uio_info;
